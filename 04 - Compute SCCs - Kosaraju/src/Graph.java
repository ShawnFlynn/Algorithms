...
public class Graph implements Iterable<Edge> {
	...
	Set<Edge> getAdjacent(int node) { 
		LinkedHashSet<Edge> arlEdges = (LinkedHashSet<Edge>) edges.get(node);
//		System.out.println(node);

		Set<Edge> tempSet = new HashSet<Edge>();
		@SuppressWarnings("unused")
		Object tempObj = arlEdges.clone();
		Iterator<Edge> tempObjItr = arlEdges.iterator();
		while (tempObjItr.hasNext()) {
			tempSet.add(tempObjItr.next());
		}
		return tempSet;
