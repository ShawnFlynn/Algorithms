...
Graph::Graph(string file_name):vertices(0),
	...
	// Get the first line of the file = size
	if (input != eos)
		vertices = *input++;

	// Check for a valid size
	if (vertices < 2) {
		cerr << "Invalid Graph: #vertices < 2" << endl << endl;
		return;
	}
	...
	// First data line flag
	bool firstDataLine = true;

	// Loop through the file
	for (string sLine; getline(data_file, sLine);) {
		...
		// Check for first data line
		if (firstDataLine) {

			// Clear the first data line flag
			firstDataLine = false;
			...
			// Set the specified tail node
			char charArray[sTemp.size()+1];
			strcpy(charArray, sTemp.c_str());
			sscanf(charArray, "%d", &tail);
		}

		// Loop through the line entries
		while (sStream >> sTemp) {

			// Head and distance variables
			int head, distance;
			...
			sscanf(charArray, "%d%c%d", &head, &comma, &distance);
