...
Graph::Graph(const int SIZE):vertices(SIZE),
	...
	// Insert the graph rows
	fill(graph.begin(), graph.end(), vector<int>(SIZE, infinity));

}	// end - Graph(size)

// Generate a 3-D graph
Graph::Graph(int i, int j, string algorithm):vertices(i),
											 edges(0),
											 pNeighbors_list(nullptr),
											 reach(nullptr) {
...
long Graph::get_APSP_edge_value(int i, int j, int k) {
...
void Graph::set_APSP_edge_value(int i, int j, int k, long dist) {
